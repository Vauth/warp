...
## 更新信息
2022.1.6  重大技术突破，绝对原创，绝对原创，绝对原创。成功把 WGCF 配置文件与环境依赖分离。本着万物皆可 Docker 的方式，以超轻量级的 Alpine 为底包（连上各依赖27MB），配置放在映射目录处，解决某些旧系统不能使用 WARP 服务的问题。后续先做完整的脚本。接着尝试把环境放到 OpenWRT ，并在后台刷 奈飞 IP，让用软路由+电视看奈飞的伙伴更爽。
