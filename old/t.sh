...
T[E126]="\$(date +"%Y-%m-%d %H:%M:%S") Try \$i. Failed. IPv\$NF: \$WAN  \$COUNTRY  \$ASNORG. Retry after \$j seconds." 
T[C126]="\$(date +"%Y-%m-%d %H:%M:%S") 尝试第\$i次，解锁失败，IPv\$NF: \$WAN  \$COUNTRY  \$ASNORG， \$j秒后重新测试"
