...
ip4_info(){
	...
	TRACE4=$(curl -s4m7 https://www.cloudflare.com/cdn-cgi/trace | grep warp | sed "s/warp=//g")
	if [[ $TRACE4 = plus ]]; then 
...
ip6_info(){
	...
	TRACE6=$(curl -s6m7 https://www.cloudflare.com/cdn-cgi/trace | grep warp | sed "s/warp=//g")
	if [[ $TRACE6 = plus ]]; then 
...
net(){
	...
	ip4_info
	[[ -n $IP4 ]] && ip6_info
	until [[ $TRACE4 =~ on|plus && $TRACE6 =~ on|plus ]]
		do	(( i++ )) || true
