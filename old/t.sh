...
# 安装 wgcf
sudo wget -N -O /usr/local/bin/aaa https://github.com/ViRb3/wgcf/releases/download/v$latest/wgcf_${latest}_linux_$architecture

# 添加执行权限
sudo chmod +x /usr/local/bin/aaa
...
sudo cp wgcf-profile.conf /etc/wireguard/aaa.conf

# 自动刷直至成功（ warp bug，有时候获取不了ip地址）
echo -e "\033[32m (3/3) 运行 WGCF \033[0m"
echo -e "\033[32m 后台获取 warp IP 中，有时候需10分钟，请耐心等待。 \033[0m"
wg-quick up aaa >/dev/null 2>&1
until [[ -n $(wget -qO- -6 ip.gs) ]]
  do
   wg-quick down aaa >/dev/null 2>&1
   wg-quick up aaa >/dev/null 2>&1
done

# 设置开机启动
systemctl enable wg-quick@aaa
