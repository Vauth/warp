...
OPTION=$1
case "$OPTION" in
1 )	[[ $PLAN = 3 ]] && yellow " 检测 WARP 已开启，自动关闭后再次运行安装 " && wg-quick down wgcf >/dev/null 2>&1 && exit
	MODIFY=$(eval echo \$MODIFYS$IPV4$IPV6);	install;;
2 )	[[ $PLAN = 3 ]] && yellow " 检测 WARP 已开启，自动关闭后再次运行安装 " && wg-quick down wgcf >/dev/null 2>&1 && exit
