...
function status(){
	...
	red "======================================================================================================================\n"
	green " 系统信息：\n	当前操作系统：$(hostnamectl | grep -i operating | awk -F ':' '{print $2}')\n	内核：$(uname -r)\n	处理器架构：$architecture\n	虚拟化：$(hostnamectl | grep -i virtualization | awk -F ': ' '{print $2}')\n	IPv4：$(wget -T1 -t1 -qO- -4 ip.gs); [[ $(wget -qO- -4 https://www.cloudflare.com/cdn-cgi/trace | grep warp ) =~ on ]] | echo (WARP IPv4)
 \n IPv6：$(wget -T1 -t1 -qO- -6 ip.gs); [[ $(wget -qO- -6 https://www.cloudflare.com/cdn-cgi/trace | grep warp ) =~ on ]] | echo (WARP IPv6) \n  $wgcf\n"
	red "======================================================================================================================\n"
		}    
