...
#	sed -i '/warp[ ]n/d' /etc/crontab
	...
	ASNORG4=$(echo $IP4 | cut -d \" -f48)
	ASNORG6=$(echo $IP6 | cut -d \" -f48)
...
#	[[ $BORINGTUN != 2 ]] && echo '@reboot sleep 10 && root bash warp n' >> /etc/crontab
	...
	[[ $TRACE6 = plus ]] && green " IPv6：$WAN6 ( WARP+ IPv6 ) $COUNTRY6 $ASNOR6 "
	[[ $TRACE6 = on ]] && green " IPv6：$WAN6 ( WARP IPv6 ) $COUNTRY6 $ASNOR6 "
	[[ $TRACE6 = off || -z $TRACE6 ]] && green " IPv6：$WAN6 $COUNTRY6 $ASNOR6 "
	end=$(date +%s)
