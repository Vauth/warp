...
function menu010(){
	...
	read -p "请输入数字:" choose010
		...
		1 ) 	dependence
			register
			sed -i '/0\.\0\/0/d' wgcf-profile.conf | sed -i 's/engage.cloudflareclient.com/162.159.192.1/g' wgcf-profile.conf |sed -i 's/1.1.1.1/9.9.9.10,8.8.8.8,1.1.1.1/g' wgcf-profile.conf 
			run;;
		2 )	dependence
			register
