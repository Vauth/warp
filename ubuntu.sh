...
# 修改配置文件 wgcf-profile.conf 的内容,使得 IPv4 的流量均被 WireGuard 接管，让 IPv4 的流量通过 WARP IPv6 节点以 NAT 的方式访问外部 IPv4 网络。为防止当节点发生故障时 DNS 请求无法发出修改为 IPv6 地址的 DNS
...
# 删除临时目录和文件
cd /root/ && rm -rf /root/warp/ /root/ubuntu.sh
